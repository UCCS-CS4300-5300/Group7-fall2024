from .models import SupportedStorageConfiguration
...
class CompatibilityService:
    ...
    def check_cpu_compatibility(build):
        ...
        if motherboard and cpu:
            if not motherboard.cpu_socket_type == cpu.socket_type:
                issues.append(
                    f"The selected CPU is not compatible with the motherboard\n"
                    f"CPU has socket type: {cpu.socket_type}\n"
                    f"Motherboard has socket type: {motherboard.cpu_socket_type}"
                )
                isValid = False
    ...
    def check_ram_compatibility(build):
        ...
        total_ram_capacity = sum(
            parse_capacity(ram_module.ram_capacity.capacity) for ram_module in ram_modules
        )
        total_ram_modules = sum(
            ram_module.ram_number_of_modules.number_of_modules for ram_module in ram_modules
        )
        ...
        if motherboard and ram_modules.exists():
            ...
            for ram_module in ram_modules:
                ...
                if motherboard_supported_rams and not any(
                    ram_module.ram_type == supported_ram.ram_type
                    for supported_ram in motherboard_supported_rams
                ):
                    issues.append(
                        f"The selected RAM is not compatible with the motherboard\n"
                        f"Motherboard supports RAM Type "
                        f"{motherboard_supported_rams[0].ram_type if motherboard_supported_rams else 'N/A'}\n"
                        f"Selected RAM has type {ram_module.ram_type}"
                    )
                ...
                supported_speeds = [
                    int(speed) for config in motherboard_supported_rams for speed in config.supported_speeds.split(',')
                ]
                if int(ram_module.ram_speed.speed) not in supported_speeds:
                    issues.append(
                        f"The selected RAM speed ({ram_module.ram_speed.speed} MHz) "
                        f"is not supported by the motherboard."
            ...
            if total_ram_capacity > motherboard.max_memory_capacity:
                issues.append(
                    f"The total RAM capacity ({total_ram_capacity} GB) exceeds the maximum capacity of the motherboard "
                    f"({motherboard.max_memory_capacity} GB)."
                )
                isValid = False

            if total_ram_modules > int(motherboard.memory_slots):
                issues.append(
                    f"The total number of RAM modules ({total_ram_modules}) exceeds the "
                    f"number of slots on the motherboard ({motherboard.memory_slots})."
                )
    ...
    def check_storage_compatibility(build):
        ...
        if motherboard and storages.exists():
            for storage_device in storages:
                supported_storage_config = (
                    SupportedStorageConfiguration.objects
                    .filter(motherboard=motherboard, storage_type=storage_device.type)
                    .first()
                )
                if not supported_storage_config:
                    issues.append(
                        f"Storage type {storage_device.type} is not supported by the motherboard."
                    )
                elif storage_device.form_factor not in supported_storage_config.supported_form_factors:
                    issues.append(
                        f"Storage form factor {storage_device.form_factor} is not supported by the motherboard."
                    )
